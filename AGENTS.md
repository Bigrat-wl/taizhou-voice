...
- 执行 prompt 一句话含 4 要素：读任务书 + 核心改动点 + 验证命令 + 不启动 dev server + **写完成报告**。

## 文档与任务书

- 任务书（诸葛产物）放 `docs/YYYY-MM-DD-<topic>-design.md`。
- 进度追踪 `docs/YYYY-MM-DD-开发进度.md`，**由诸葛统一维护**，子 agent 不碰。
- 长期参考文档：`docs/操作手册.md`（命令/流程）、`docs/API契约.md`（接口对齐）、`docs/数据模型.md`（表结构）。

## 汇报日志

- 一天分 **3 个时段**（24 小时制，时间边界固定）：
  - 上午：06:00 – 12:00
  - 下午：12:00 – 18:00
  - 晚上：18:00 – 次日 06:00
- 每时段一个文档：`docs/reports/YYYY-MM-DD-<时段>.md`；跨午夜的「晚上」按工作开始的日期归属。
- 子 agent 在**当前时段**把执行情况追加到对应时段的文档，**只追加不改动**上方已有内容。
- **分类**：同一文档内按任务分类记录（`## <任务名>` + 做了什么 / 改动文件 / 验证结果 / 遗留问题）。
- **格式**：每个会话产物前加 `---` 分割线，分割线下方标完成时间点（斜体小字 `*HH:MM*`），时间点紧挨 `## 任务名` 标题上方。
- 子 agent 不更新进度文档，进度由诸葛 review 后统一维护。
