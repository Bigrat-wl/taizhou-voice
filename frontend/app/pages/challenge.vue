...
<template>
  <div class="flex h-full flex-col px-4 pb-4 max-w-5xl mx-auto w-full">
    <!-- 页头（紧凑） -->
    <section class="py-3 text-center shrink-0">
      <span class="mx-auto flex h-8 w-8 items-center justify-center rounded-lg bg-amber-100 text-amber-600">
        <Icon :name="item.icon" class="h-4 w-4" />
      </span>
      <h1 class="mt-1 text-xl font-bold text-slate-900">{{ item.label }}</h1>
    </section>

    <!-- 未登录提示（shrink-0） -->
    <div v-if="!isLoggedIn" class="mb-3 shrink-0 rounded-xl border border-amber-200 bg-amber-50 p-3 text-center">
    ...
    <!-- 进度条（shrink-0） -->
    <div v-if="sentences.length > 0 && !allDone" class="mb-3 shrink-0">
    ...
    <div class="flex flex-1 min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div class="flex-1 min-h-0 overflow-y-auto p-5">
        ...
        <!-- 加载中 -->
        <div v-if="loadingSentences" class="flex flex-col items-center justify-center py-12">
        ...
        <div class="min-h-[3rem]">
          <p v-show="sentencesError" class="rounded-lg bg-rose-50 px-4 py-3 text-center text-sm text-rose-600">
            {{ sentencesError }}
          </p>
        </div>
          ...
          >
            ...
            <div class="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-1.5 pl-9 text-sm text-slate-500">
              ...
              <div class="flex h-14 w-14 shrink-0 items-center justify-center">
                <div
                  v-show="getState(sentence.id).score != null"
                  class="flex h-14 w-14 items-center justify-center rounded-full text-xl font-bold text-white shadow"
                  :class="scoreBadgeColor(getState(sentence.id).score ?? 0)"
                >
              ...
              <div class="flex flex-col gap-3">
                ...
                <template v-if="getState(sentence.id).phase !== 'scored'">
                  ...
                  <div class="flex flex-wrap items-center gap-3">
                    ...
                    <!-- 提交评分 -->
                    <button
                      v-show="(getState(sentence.id).phase === 'recorded' || getState(sentence.id).phase === 'submitting') && getState(sentence.id).blob"
                      type="button"
                  ...
                  <div class="min-h-[1.25rem]">
                    <p
                      v-show="recorderError && activeId === sentence.id"
                  ...
                  <!-- 句子级错误：固定高度容器 -->
                  <div class="min-h-[1.25rem]">
                    <p v-show="getState(sentence.id).error" class="text-sm text-rose-500">
                  ...
                  <div class="min-h-[4.5rem]">
                    ...
                    >
                      <p class="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-slate-500">
                ...
                <template v-if="getState(sentence.id).phase === 'scored' && getState(sentence.id).score != null">
                  ...
                  <div class="flex flex-col items-center py-4">
                    <div
                      class="flex h-20 w-20 items-center justify-center rounded-full text-3xl font-black text-white shadow-lg"
                  ...
                  <div class="flex flex-wrap items-center gap-3 pt-1">
                    ...
                    <button
                      v-if="!isLastSentence"
                      type="button"
                      class="flex flex-1 items-center justify-center gap-2 rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700"
                      @click="goNext"
                    >
                      下一句
                    ...
                    <button
                      type="button"
                      class="flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100"
                      @click="
                        (() => {
                ...
                <!-- 句子级错误（scored 态兜底）：固定高度容器 -->
                <div class="min-h-[1.25rem]">
                  <p
                    v-show="getState(sentence.id).phase === 'scored' && getState(sentence.id).error"
                    class="text-sm text-rose-500"
                  >
                    {{ getState(sentence.id).error }}
                  </p>
        ...
        <!-- ━━━ 总结页（全部完成） ━━━ -->
        <div v-if="allDone" class="flex flex-col items-center gap-5 py-6">
          <!-- 总分大字 -->
          ...
          <!-- 平均分 -->
          <div class="flex items-center gap-3 rounded-2xl border bg-white px-6 py-4 shadow-sm">
            <div
              class="flex h-14 w-14 items-center justify-center rounded-full text-xl font-bold text-white shadow"
          ...
          <!-- 每句得分概览 -->
          <div class="w-full rounded-2xl border bg-white p-4 shadow-sm">
            <h3 class="mb-2.5 text-sm font-semibold text-slate-700">各句得分</h3>
          ...
          <div class="flex gap-3">
            <button
              type="button"
              class="flex items-center gap-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700"
            ...
            <NuxtLink
              to="/leaderboard"
              class="flex items-center gap-2 rounded-lg border border-slate-300 px-6 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100"
            >
        ...
        <!-- 底部操作（未全部完成时的换一批） -->
        <div v-if="sentences.length > 0 && !allDone" class="mt-6 flex justify-center">
          <button
            type="button"
            class="flex items-center gap-2 rounded-lg border border-slate-300 px-5 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-100"
            @click="loadSentences"
          >
            <Icon name="lucide:refresh-cw" class="h-4 w-4" />
            换一批句子
